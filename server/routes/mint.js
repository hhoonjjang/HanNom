...
const web3 = new Web3('wss://goerli.infura.io/ws/v3/2370d723f2b24ee69ca1d052c7a0e099');
dotenv.config();
    ...
    try {
        ...
        if (curheight == null) {
            for (let i = 0; i < tempArr.length; i++) {
                ...
                //   )
                // ).data;
                await axios.get(tempArr[i].tokenURI).then(async ({ data }) => {
                    console.log(data);
                    console.log('아이', i + 1);
...
router.post('/registList', async (req, res) => {
    ...
    try {
        ...
        if (tempArr.length > 0) {
            for (let i = 0; i < tempArr.length; i++) {
                const { name, description, image } = (await axios.get(tempArr[i].tokenURI)).data;
                data.push({
...
router.post('/cancel', async (req, res) => {
    const deployed = new web3.eth.Contract(SaleAbi.abi, process.env.SALE_CA);
    try {
        const cancel = await deployed.methods.cancelSaleToken(req.body.tokenId).encodeABI();
        const obj = {
            to: '',
            from: '',
            data: '',
...
router.post('/detail', async (req, res) => {
    ...
    try {
        ...
        const { name, description, image } = (await axios.get(nft.tokenURI)).data;
...
router.post('/sell', async (req, res) => {
    ...
    const nft = await deployed.methods.SalesToken(req.body.tokenId, price).encodeABI();
    const approve = await deployedN.methods.setApprovalForAll(process.env.SALE_CA, true).encodeABI();
    const approveObj = {
        to: '',
        from: '',
        data: '',
...
router.post('/buy', async (req, res) => {
    ...
    // console.log(temp);
    try {
        const buy = await deployed.methods.PurchaseToken(req.body.tokenId).encodeABI();
        const obj = {
            to: '',
            from: '',
            data: '',
...
router.post('/sellComplete', async (req, res) => {
    try {
        ...
        const data = await Nft.update({ state: 'selling' }, { where: { tokenId: req.body.tokenId } });
...
router.post('/cancelComplete', async (req, res) => {
    try {
        console.log(req.body.tokenId);
        const data = await Nft.update({ state: 'mint' }, { where: { tokenId: req.body.tokenId } });
...
router.post('/mintComplete', async (req, res) => {
    ...
    try {
        ...
        let tokenIdx;
        console.log('드가자');
        const { name, description, image, upload } = (await axios.get(tempArr[tempArr.length - 1].tokenURI)).data;
        console.log('드가자');
...
router.post('/tradeList', async (req, res) => {
    ...
    try {
        ...
        const list = await TradeHistory.findAll({
            where: {
                currency: '거래완료',
                [Op.or]: [{ buyerAddress: req.body.address }, { sellerAddress: req.body.address }],
            },
            include: [
                {
                    model: Nft,
